...
export default class MiriamAnimation extends THREE.Object3D {
    constructor( scene, renderer ) {
        ...
        // Shaders
        this.render_fs = glslify('../shaders/render_fs.glsl');
        this.render_vs = glslify('../shaders/render_vs.glsl');
    ...
    setupAnim() {
        ...
        // CREATE CURVE
        let manMaterial = new THREE.MeshBasicMaterial({color: 0xcccccc, morphTargets: true, transparent: true, opacity: 0});
    ...
    loadModelClock (model, modelB, modelC, modelD, meshMat) {
        ...
        loader.load(modelD, (geometryD, material)=>{
            ...
            this.grandFatherClock.scale.multiplyScalar(0.06);
            // this.grandFatherClock.position.set(2, 3, -3.5);
            this.grandFatherClock.position.set(0, 8, 1);
            ...
            for(let i=0; i<this.sequenceConfig.length; i++){
                ...
                if(animTime >= this.sequenceConfig[i].time && !this.sequenceConfig[i].performed){
                    ...
                    if( i<this.manGeometries.length ){
                        ...
                        this.timerAnim = TweenMax.fromTo(this.simulationShader.uniforms.timer, 8, {value:0}, {value:1, ease: Power4.easeInOut});
                    }
