...
export default class MiriamAnimation extends THREE.Object3D {
    ...
    initParticles( geo ) {
        ...
        this.disposeRelatedGeos.push(fboGeo);
    ...
    setupAnim() {        
        ...
        // DebugUtil.positionObject(this, "Miriam Ani");

        // setup animation sequence
        // time: when to happen, duration: how long / fast the animation is
        this.animStart = true;
        this.sequenceConfig = [
            { time: 2, anim: ()=>{this.manOn()} },
            { time: 5, anim: ()=>{this.fboOn()} }, 
            ...
            { time: 115, anim: ()=>{this.manSwirl(8)} },            //115    //45
            { time: 125, anim: ()=>{this.manSwirl2(8)} },           //125    //55
            { time: 135, anim: ()=>{this.manSwirl3(8)} },           //135    //65
            { time: 145, anim: ()=>{this.manSwirlNonstop()} },      //145    //75
            { time: 220, anim: ()=>{this.manSwirlSpeedup(46)} }     //220, 20    //90
        ];

        this.GFClockTex = tex_loader.load(this.BASE_PATH + '/images/clockUV4.jpg');
        this.GFClockMat = new THREE.MeshLambertMaterial( {map: this.GFClockTex} );
        ...
        // CREATE CURVE
        this.manMaterial = new THREE.MeshLambertMaterial({color: 0xff0000, morphTargets: true, transparent: true, opacity: 1});
        ...
        this.manFigure = new THREE.Mesh(manGeometry, this.manMaterial);
        ...
        // HIDE
        this.manFigure.scale.multiplyScalar(0.01);
        this.manFigure.visible = false;
    ...
    manSwirlSpeedup (_duration) {
        ...
        TweenMax.to( this.dummy, _duration, { timeScaleValue: 30,
                                       ease: Power2.easeIn,
                                       onStart: ()=>{
                                                    TweenMax.to( this.manFigure.scale, 5, { x:1.2,y:1.2,z:1.2, ease: Power2.easeInOut } );
    ...
    loadModelClock (model, modelB, modelC, modelD, meshMat) {
        ...
        loader.load(modelC, (geometryC, material)=>{
            geometryC.center();
            this.disposeRelatedGeos.push(geometryC);
        ...
        loader.load(modelD, (geometryD, material)=>{
            geometryD.center();
            this.disposeRelatedGeos.push(geometryD);
            ...
            for(let i=0; i<6; i++){
                ...
                this.disposeRelatedGeos.push(geoTemp);
    ...
    disposeAni(){
        // clock
        this.remove(this.grandFatherClock);
        this.GFClockMat.dispose();
        this.GFClockTex.dispose();

        // figure
        this.remove( this.manFigure );
        for(var i=0; i<this.manGeometries.length; i++){ this.manGeometries[i].dispose(); }
        this.manMaterial.dispose();
        ...
        console.log("dispose Miriam ani!");
    }
    ...
    updateMorphForFBO(geo, speedUp) {
        ...
        {
            this.timerAnim = TweenMax.fromTo(this.simulationShader.uniforms.timer, timerSpeed, {value:0}, {value:0.5, ease: Expo.easeIn});
        }
        else
        {
            this.timerAnim = TweenMax.fromTo(this.simulationShader.uniforms.timer, timerSpeed, {value:0}, {value:0.5}); //, ease: Expo.easeOut
        }
    }
    ...
    updateVideoTime(time) {
        if (this.nextAnim && time >= this.nextAnim.time) {
            ...
            // FBO
                ...
                if ( index>1 && (index-2) < this.manGeometries.length ) {
                    this.updateMorphForFBO( this.manGeometries[index-2], 0 );
                }
