...
export default class Lupo12PMAnimation extends THREE.Object3D {
    ...
    setupAnim() {
        ...
        // CACTUS!
            ...
            this.cactusLoadingManager.onLoad = () => {
                ...
                for(let i=0; i<10; i++){
                    ...
                    this.disposeRelatedMats.push(new_mat);
            ...
            this.cactusTexLight = p_tex_loader.load( this.BASE_PATH + "/images/cactus_white.jpg" );
            this.cactusTex = p_tex_loader.load( this.BASE_PATH + "/images/cactusTex.jpg" );
            this.cactusMat = new THREE.MeshLambertMaterial({map: this.cactusTexLight, color: 0x1fc529, side: THREE.DoubleSide}); //0x298a59
            this.sculptCactusMat = new THREE.MeshLambertMaterial({map: this.cactusTex, color: 0xcccccc, side: THREE.DoubleSide}); //0x206c5e
        ...
        // Ropes to dogs!
            ...
            for(let i=0; i<2; i++){
                ...
                this.liquidTexs.push(liquidTex);
    ...
    createRope( _index ) {
        ...
        this.disposeRelatedGeos.push(ropeGeometry);
        ...
        this.disposeRelatedGeos.push(liquidGeo);
    ...
    disposeAni(){
        // sculptures
        this.remove(this.lupoArt);
        this.baseMat.dispose();
        this.bottomMat.dispose();
        this.topMat.dispose();
        for(var i=0; i<this.sculptureMaterials.length; i++){ this.sculptureMaterials[i].dispose(); }
        for(var i=0; i<this.sculptureMaterials.length; i++){ this.sculptureMaterials[i].dispose(); }
        for(var i=0; i<this.liquidMats.length; i++){ this.liquidMats[i].dispose(); }

        // cactus
        this.remove(this.cactusGroup);
        this.remove(this.cactusGroupFloor);
        this.cactusMat.dispose();
        this.sculptCactusMat.dispose();
        this.cactusTexLight.dispose();
        ...
        // ropes
        for(var i=0; i<this.ropes.length; i++){ this.remove(this.ropes[i]); }

        for(var i=0; i<this.liquidMats.length; i++){ this.liquidMats[i].dispose(); }
        this.ropeMaterial.dispose();
        for(var i=0; i<this.liquidTexs.length; i++){ this.liquidTexs[i].dispose(); }
    ...
    loadSculptureTextures ( textureFiles, textureMADFiles ) {
        let promise = new Promise( (resolve, reject) => {
            ...
            for(let i=0; i<textureFiles.length; i++){
                ...
                this.disposeRelatedTexs.push(_tex);
    ...
    reset() {
        ...
        // RESET CACTUS
        for(let i=0; i<this.cactusTimelines.length; i++){
            this.cactusTimelines[i].pause(0);
        }
        for(let i=0; i<this.cactusFloorTimelines.length; i++){
            this.cactusFloorTimelines[i].pause(0);
        }
        for(let i=0; i<this.flowerTimeline.length; i++){
            this.flowerTimeline[i].pause(0);
        }
        for(let i=0; i<this.cactusGroup.children.length; i++){
            for(let j=0; j<this.cactusGroup.children[i].length; j++){
                this.cactusGroup.children[i].children[j].scale.set(0.01, 0.01, 0.01);
                this.cactusGroup.children[i].children[j].visible = false;
            }
        }
        for(let i=0; i<this.cactusGroupFloor.children.length; i++){
            for(let j=0; j<this.cactusGroupFloor.children[i].length; j++){
                this.cactusGroupFloor.children[i].children[j].scale.set(0.01, 0.01, 0.01);
                this.cactusGroupFloor.children[i].children[j].visible = false;
            }
        }
        for(let i=0; i<this.flowerGroup.length; i++){
            for(let j=0; j<this.flowerGroup[i].children.length; j++){
                this.flowerGroup[i].children[j].children[0].scale.set(0.01, 0.01, 0.01);
            }
        }
        ...
        this.liquidOut = false;
        for(let i=0; i<this.ropes.length; i++){
            this.ropes[i].children[0].material.map.offset.x = -1.5;
        }
    }
